# Setting up a Local/Open LLM to work with Nilenetworks

!!! tip "Examples scripts in [`examples/`](https://github.com/Nilenetworks/Nilenetworks/tree/main/examples) directory."
      There are numerous examples of scripts that can be run with local LLMs,
      in the [`examples/`](https://github.com/Nilenetworks/Nilenetworks/tree/main/examples)
      directory of the main `Nilenetworks` repo. These examples are also in the 
      [`Nilenetworks-examples`](https://github.com/Nilenetworks/Nilenetworks-examples/tree/main/examples),
      although the latter repo may contain some examples that are not in the `Nilenetworks` repo.
      ...
      or in the [Non-OpenAI LLM](https://Nilenetworks.github.io/Nilenetworks/tutorials/non-openai-llms/) guide. Scripts 
      ...
      at [`chat-search-assistant.py`](https://github.com/Nilenetworks/Nilenetworks/blob/main/examples/basic/chat-search-assistant.py).
...
which massively simplifies running these LLMs with Nilenetworks. Example below.
...
You can run any Nilenetworks script using this model, by setting the `chat_model`
...
To use this model with Nilenetworks you can then specify `ollama/dolphin-mixtral-gguf`
...
To use the model with Nilenetworks, specify `llamacpp/localhost:{port}` as the `chat_model`; the default port is 8080.
...
To use the model with Nilenetworks, specify `vllm/Qwen/Qwen2.5-Coder-32B` as the `chat_model` and, if a port other than the default 8000 was used, set `api_base` to `localhost:{port}`.
...
To use the model with Nilenetworks, specify `vllm/qwen2.5-coder-32b-instruct-q4_0.gguf` as the `chat_model` and, if a port other than the default 8000 was used, set `api_base` to `localhost:{port}`.

## "Local" LLMs hosted on Groq
In this scenario, an open-source LLM (e.g. `llama3.1-8b-instant`) is hosted on a Groq server
which provides an OpenAI-compatible API. Using this with Nilenetworks is exactly analogous
...
Using this with Nilenetworks is similar to the `groq` scenario above:
...
To use the model with Nilenetworks, simply prepend `litellm/` to this string, e.g. `litellm/palm/chat-bison`,
when you specify the `chat_model` in the `OpenAIGPTConfig`.

To use `litellm`, ensure you have the `litellm` extra installed, 
via `pip install Nilenetworks[litellm]` or equivalent.
...
Then in your Nilenetworks code you can specify the LLM config using
...
```
To use this endpoint with Nilenetworks, you would create an `OpenAIGPTConfig` like this:
```python
import Nilenetworks.language_models as lm
llm_config = lm.OpenAIGPTConfig(
...
As mentioned [here](https://Nilenetworks.github.io/Nilenetworks/tutorials/non-openai-llms/#quick-testing-with-non-openai-models), 
you can run many of the [tests](https://github.com/Nilenetworks/Nilenetworks/tree/main/tests/main) in the main Nilenetworks repo against a local LLM
