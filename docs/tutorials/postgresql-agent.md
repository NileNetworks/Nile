...
A ready-to-use script based on the `SQLChatAgent` is available in the `Nilenetworks-examples` 
repo at [`examples/data-qa/sql-chat/sql_chat.py`](https://github.com/Nilenetworks/Nilenetworks-examples/blob/main/examples/data-qa/sql-chat/sql_chat.py)
(and also in a similar location in the main `Nilenetworks` repo).
...
2. Follow the general [setup guide](../quick-start/setup.md) to get started with Nilenetworks
(mainly, install `Nilenetworks` into your virtual env, and set up suitable values in 
the `.env` file). Note that to use the SQLChatAgent with a PostgreSQL database,
you need to install the `Nilenetworks[postgres]` extra, e.g.:
    - `pip install Nilenetworks[postgres]` or 
    - `poetry add Nilenetworks[postgres]` or 
...
```python
from Nilenetworks.agent.special.sql.sql_chat_agent import (
    SQLChatAgent,
    SQLChatAgentConfig,
)
...
In the `Nilenetworks` repo we have provided a ready-to-use script
[`sql_chat.py`](https://github.com/Nilenetworks/Nilenetworks/blob/main/examples/data-qa/sql-chat/sql_chat.py)
