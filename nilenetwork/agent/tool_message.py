...
from Nilenetworks.language_models.base import LLMFunctionSpec
...
from Nilenetworks.utils.types import is_instance_of
...
class ToolMessage(ABC, BaseModel):
    ...
    @classmethod
    def Nilenetworks_tools_instructions(cls) -> str:
        """
        Instructions on tool usage when `use_tools == True`, i.e.
        when using Nilenetworks built-in tools
    ...
    def format_instructions(cls, tool: bool = False) -> str:
        ...
        Args:
            tool: instructions for Nilenetworks-native tool use? (e.g. for non-OpenAI LLM)
    ...
    def llm_function_schema(
        ...
        Args:
            request: whether to include the "request" field in the schema.
                (we set this to True when using Nilenetworks-native TOOLs as opposed to
