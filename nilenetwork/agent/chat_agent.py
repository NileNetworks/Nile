...
from Nilenetworks.agent.base import Agent, AgentConfig, async_noop_fn, noop_fn
from Nilenetworks.agent.chat_document import ChatDocument
from Nilenetworks.agent.tool_message import (
    ToolMessage,
    format_schema_for_strict,
)
from Nilenetworks.agent.xml_tool_message import XMLToolMessage
from Nilenetworks.language_models.base import (
    LLMFunctionCall,
    LLMFunctionSpec,
    LLMMessage,
    LLMResponse,
...
from Nilenetworks.language_models.openai_gpt import OpenAIGPT
from Nilenetworks.pydantic_v1 import BaseModel, ValidationError
from Nilenetworks.utils.configuration import settings
from Nilenetworks.utils.object_registry import ObjectRegistry
from Nilenetworks.utils.output import status
from Nilenetworks.utils.pydantic_utils import PydanticWrapper, get_pydantic_wrapper
...
class ChatAgentConfig(AgentConfig):
    ...
    Attributes:
        ...
        use_tools_on_output_format: Controls whether to automatically switch
            to the Nilenetworks-native tools mechanism when `output_format` is set.
    ...
    def _set_fn_or_tools(self, fn_available: bool) -> None:
        """
        Enable Nilenetworks Tool or OpenAI-like fn-calling,
        ...
        if self.use_functions_api and not fn_available:
            logger.debug(
                ...
                Nilenetworks's ToolMessage mechanism.
                """
            )
...
class ChatAgent(Agent):
    ...
    def __init__(
        ...
        # (b) these are only for the builtin in Nilenetworks TOOLS mechanism:
        ...
        if self.config.enable_orchestration_tool_handling:
            ...
            from Nilenetworks.agent.tools.orchestration import (
                AgentDoneTool,
                AgentSendTool,
                DonePassTool,
                DoneTool,
        ...
        for msg_cls in enabled_classes:
            if msg_cls.default_value("request") in self.llm_tools_usable:
                ...
                if (
                    ...
                    and hasattr(msg_cls, "Nilenetworks_tools_instructions")
                    and inspect.ismethod(msg_cls.Nilenetworks_tools_instructions)
                ):
                    class_instructions += msg_cls.Nilenetworks_tools_instructions()
    ...
    def enable_message(
        ...
        if isinstance(message_class, XMLToolMessage):
            ...
            # so we disable use of functions API, enable Nilenetworks-native Tools,
    ...
    def set_output_format(
        ...
        switches to the native Nilenetworks tools mechanism to ensure that no tool
